...
function TForm2.GetUserInfoMsg(AName: string; AAge: Integer; AIsMan: Boolean): string;  //성별확인 함수 코딩
var
  Msg, Gender: string; //메세지, 성별
begin
  Msg := GetAgeMsg(AName,AAge) + #13#10;  //함수 및 줄바꿈.
  ...
  Msg := Msg + AName + '님은 ' + Gender + '입니다.';    //메세지 출력.
  Result := Msg; //반환값
end;
...
begin
  ...
  if edtAge.Text = '' then    //나이를 입력 안했을시
  begin            //오류메세지 표시
