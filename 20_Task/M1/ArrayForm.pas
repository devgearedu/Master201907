...
type
  ...
  private
    FNumArr: array[0..9] of Integer; //10개의 배열 생성
...
begin
  ...
  for I := 0 to Length(FNumArr) - 1 do //변수 := (초기값) to 배열 - 1
    FNumArr[I] := Random(100); //For문으로 배열에 0~100중 1개씩 삽입
end;
...
begin
  ...
  { TODO :
    (3) for 문을 이용해 배열의 값 중 가장 큰 값을 반환하도록 구현
        if 문을 이용해 숫자를 비교  }
  for I := 0 to Length(FNumArr) -1 do  //배열 안의 숫자를 하나씩 비교
...
begin
  ...
  for I := 0 to Length(FNumArr) -1 do
    if FNumArr[I] >= 50 then   //50이상일시
      Inc(CountOver) // CountOver변수 1씩 증가
    else                       //50미만일시
      Inc(CountUnder);
