...
uses DataAccessModule, CommonFunctions, FindBookForm, FindUserForm;
...
begin
  ...
  try
    ...
    ShowMessage(frmFindBook.SelectedSeq.ToString);
  finally
    frmFindBook.Free;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if Filter <> '' then
        Filter := Filter + ' or ';
      Filter := Filter + Format('USER_NAME like ''%%%s%%''', [edtSearch.Text]);
    end;
  end;
